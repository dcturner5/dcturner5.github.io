...
function leaveFaction($conn) {
    $token = getToken();
    $sql = 'DELETE FROM `factions_users` WHERE users_id = (SELECT u.id FROM `users` u WHERE u.token = ?) AND rank != "boss"';
    $stmt = $conn->prepare($sql);
    if($stmt->execute(array($token))) {
        return true;
    }
